...
class Trendline_estimator:
	...
	def __linear_fit_slope(self):  #
		...
		if denominator != 0:
			logging.info("sample of accDelay: %s", self.smoothed_delay_list)
			logging.info("sample of arrivalTime: %s", self.arrival_delta_list)
	...
	def trendline_update(self, recv_delta_ms, send_delta_ms, arrival_ts):
		...
		self.smoothed_delay_list[
			hist_index] = self.smoothed_delay
		
		if self.index >= self.window_size:
			self.trendline = self.__linear_fit_slope()
		
		# print("trendline", self.trendline)
		logging.info("trend is [%s]", self.trendline)
		return
	
	def trendline_slope(self):
		# logging.info("queueDelayDelta is [%s]", self.threshold_gain * self.trendline)
