...
class PPO:
	...
	def update(self, storage, state):
		...
		advantages = (advantages - advantages.mean()) / (advantages.std() + 1e-5)  # 规范化优势函数
		
		old_states = torch.squeeze(torch.stack(storage.states).to(self.device),
		                           1).detach()  ##squeeze是对数据进行压缩，去掉维度为1 的维度；stack是数据的拼接
