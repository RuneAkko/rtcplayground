...
class HrccGCCEstimatorWithKalman(object):
	...
	def overuse_detector(self, queueDelayDelta, ts_delta):
		"""
		...
		if queueDelayDelta > self.gamma1:
			...
			if self.time_over_using > kOverUsingTimeThreshold and self.overuse_counter > 1:
				if queueDelayDelta > self.prev_queueDelayDelta:
					self.time_over_using = 0
					self.overuse_counter = 0
					self.overuse_flag = 'OVERUSE'
		elif queueDelayDelta < -self.gamma1:
			self.time_over_using = -1
			self.overuse_counter = 0
		...
		self.prev_queueDelayDelta = queueDelayDelta
		self.update_threthold(queueDelayDelta, now_ms)
