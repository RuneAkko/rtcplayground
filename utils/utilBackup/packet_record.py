...
# class RateControler:
#     def __init__(self,estimated_rate = 50000):
#         self.bwe = estimated_rate
#         self.packet_record = None
#         self.pre_state = None
#         self.pre_bwe = 50000
#         self.inc_coff = 1.05
#         self.dec_coff = 0.85
...
#         if signal == kBwNormal:
#             if self.pre_state == kRcDecrease:
#                 self.pre_state = kRcHold
#             else:
#                 self.pre_state = kRcIncrease
#         elif signal == kBwUnderusing:
#             self.pre_state = kRcHold
#         else:
#             #OverUsing
#             self.pre_state = kRcDecrease
#         self.change_bitrate()

#     def change_bitrate(self):
#         self.pre_bwe = self.bwe
#         if self.pre_state == kRcIncrease:
#             self.bwe *= self.inc_coff
#         elif self.pre_state == kRcDecrease:
#             self.bwe *= self.dec_coff

#     def get_estmiated_rate(self):
#         return self.bwe
kBwNormal = 200
kBwOverusing = 201
kBwUnderusing = 202
...
class RateControler:
	def __init__(self, estimated_rate=50000, max_bitrate=300000000, min_bitrate=30000):
		self.bwe = estimated_rate
		self.packet_record = None
		self.state = None
		self.pre_bwe = 50000
	...
	def change_bitrate(self):
		self.pre_bwe = self.bwe
		if self.state == kRcIncrease:
			self.bwe *= self.inc_coff
		
		elif self.state == kRcDecrease:
			# self.bwe = self.dec_coff * self.packet_record.calculate_receiving_rate(500)
			self.bwe *= self.dec_coff
		self.bwe = min(self.max_bitrate, self.bwe)
		self.bwe = max(self.min_bitrate, self.bwe)
